...
class InfluxDBBadRequestError(InfluxDBError):
    MESSAGE_PLACEHOLDER = 'Bad request for params : {params} with message {message}'

    def __init__(self, params, message):
        self.message = self.MESSAGE_PLACEHOLDER.format(params=params, message=message)
        super().__init__(self.message)
...
class InfluxDBInvalidBooleanError(InfluxDBError):
    MESSAGE_PLACEHOLDER = 'Invalid boolean : {points}'
