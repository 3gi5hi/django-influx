...
class TimestampFieldAttribute(BaseAttribute):
    ...
    def clean(self, value):
        ...
        if value is None and self.auto_now:
            ...
            timestamp = self._value.timestamp
    ...
    def to_influx(self, value):
        self.formatted_timestamp = int(self.formatted_timestamp)
...
class DateTimeFieldAttribute(TimestampFieldAttribute):
    def __init__(self, **kwargs):
        self.str_format = kwargs.get('str_format', "%Y-%m-%d %H:%M:%S")
    ...
    def get_internal_value(self):
        ...
        if isinstance(self._value, datetime):
            return self._value.strftime(self.str_format)
        try:
            dt = datetime.fromtimestamp(self._value).strftime(self.str_format)
        except:
            dt = datetime.strptime(self._value, self.str_format)
        return dt
    ...
    def to_influx(self, value):
        ...
        if isinstance(value, datetime):
            ...
            if value.isnumeric():
                ...
                timestamp = datetime.strptime(value, self.str_format).timestamp()
    ...
    def to_python(self, value):
        if isinstance(value, datetime):
            ...
            try:
                ...
                dt = datetime.strptime(value, self.str_format)
        else:
            dt = value
        return dt
