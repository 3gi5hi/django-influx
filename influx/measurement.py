...
class MeasurementMeta(type):
    ...
    def _factory_get_query(cls):
        def get_query(from_db=None):
            class MeasurementQuery(Query):
                def __init__(self):
                    db = from_db or cls.database
                    super(MeasurementQuery, self).__init__(db=db)

                def format(self, result, parser_class=cls.parser_class, **kwargs):
                    return parser_class(result, cls).convert()

                def evaluate(self, parser_class=cls.parser_class, **kwargs):
                    return super(MeasurementQuery, self).evaluate(parser_class=parser_class, **kwargs)
    ...
    def _extend_attributes(cls, attribute_names):
        ...
        for attribute_name in attribute_names:
            ...
            #     attribute_name = attribute_field.attribute_name
            # ext_attribute_name = EXTENDED_ATTRIBUTE_PREFIX_NAME + attribute_name
            if attribute_field.attribute_name is None:
...
class Measurement(object, metaclass=MeasurementMeta):
    ...
    def __repr__(self):
        ...
        if hasattr(self, 'time'):
            timestamp = self.time
            name = '{} at {}'.format(name, timestamp)
        return '<Measurement {}>'.format(name)
    ...
    @property
    def timestamp(self):
        if type(getattr(self, 'time', None)) in [int, float]:
            return datetime.fromtimestamp(getattr(self, 'time'), timezone.utc)
    ...
    def save(self, db=None):
        ...
        if hasattr(self, 'time') and getattr(self, 'time', None) is None:
            delattr(self.__class__, 'time')
        str_points = self.get_prep_value() + "\n"
        return BulkInsertQuery(str_points, db=database).execute()

    @classmethod
    def bulk_save(cls, points, db=None):
        ...
        for point in points:
            ...
            if hasattr(point, 'time') and getattr(point, 'time', None) is None:
                delattr(point.__class__, 'time')
...
def SimpleMeasurement(measurement_name, field_names, tag_names=[], database=None, precision=None):
    ...
    parsed_tags = {tag.replace('-', '_'): tag for tag in tag_names}
    parsed_fields = {field.replace('-', '_'): field for field in field_names}
    rendered_template = j2_env \
        ...
        database=database or settings.INFLUXDB['NAME'],
        precision=precision
    )
